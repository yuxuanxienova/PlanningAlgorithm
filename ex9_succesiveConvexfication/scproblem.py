...
class SCProblem:
    ...
    def __init__(self, m, K):
        ...
        self.par["x_init"] = cvx.Parameter(m.n_x)
        self.par["goal_state"] = cvx.Parameter(6)
        self.par["time_past"] = cvx.Parameter(nonneg=True)

        if m.n_sat > 0:
            self.par["sats_pos"] = cvx.Parameter((K * 2, m.n_sat), name="sats_pos")
        else:
            self.par["sats_pos"] = cvx.Parameter(nonneg=True)  # dummy
        ...
        # Boundary conditions:
        constraints += [
            self.var["X"][:, 0] == self.par["x_init"],
            self.var["X"][0:6, -1] == self.par["goal_state"],
        ]

        # Dynamics:
        constraints += [
            self.var["X"][:, k + 1]
            == cvx.reshape(self.par["A_bar"][:, k], (m.n_x, m.n_x))
            @ self.var["X"][:, k]
            + cvx.reshape(self.par["B_bar"][:, k], (m.n_x, m.n_u)) @ self.var["U"][:, k]
            + cvx.reshape(self.par["C_bar"][:, k], (m.n_x, m.n_u))
            @ self.var["U"][:, k + 1]
        ...
        constraints += [
            cvx.norm(dx, 1) + cvx.norm(du, 1) + cvx.norm(ds, 1) <= self.par["tr_radius"]
        ]

        # Objective:
        model_objective = m.get_objective(
        ...
        objective = (
            sc_objective if model_objective is None else sc_objective + model_objective
        )
