...
class FirstOrderHold:
    ...
    def calculate_discretization(self, X, U, sigma):
        ...
        for k in range(self.K - 1):
            ...
            self.B_bar[:, k] = np.matmul(
                Phi, V[self.B_bar_ind].reshape((self.n_x, self.n_u))
            ).flatten(order="F")
            self.C_bar[:, k] = np.matmul(
                Phi, V[self.C_bar_ind].reshape((self.n_x, self.n_u))
            ).flatten(order="F")
    ...
    def _ode_dVdt(self, V, t, u_t0, u_t1, sigma):
        ...
        dVdt[self.A_bar_ind] = np.matmul(
            A_subs, V[self.A_bar_ind].reshape((self.n_x, self.n_x))
        ).reshape(-1)
